	    ...
	    $.ajax({
	        ...
	        success:function(serverData){   //服务端返回200状态码时的回调函数ss
              ...
              var  userinfo1={
                ...
                $.ajax({
                    type:"POST",
                    url:"findArticleByarticle_colectUserIdServlet",
                    data:"userRegInfo_all="+JSON.stringify(userinfo1),
                    dataType:"json",
                    beforeSend:function(){
                       // alert("弹出加载动画2");
                    ...
                    //    console.log(arts[i])
                     ///   console.log("----------------------------------------")
                        out =  '<div class="grid-item">\n'+
                                ...
                                '                           <a class="md-btn md-btn--outline-primary" href="'+encodeURI("colectblog-detail.html?"+'contenturl='+arts[i].archtml+'&date='+arts[i].arctime+'&title='+arts[i].title+'&user='+arts[i].userid+'&arcid='+arts[i].arcid)+'">read more\n'+
                                '                           </a>\n'+
                                '                       </div>\n'+
                                '                   </div>\n'+
                                '                </div>\n'+
